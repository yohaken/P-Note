...
|---|---|---|---|
| Backend API | `backend/` | `cd backend && npm run dev` | Express, `node --watch`, `:8080`. Endpoints: `GET /api/health`, `/api/version`, `/api/db-status`, `GET/PUT /api/spaces/:spaceId/notes` (Firestore). For local dev, start the Firestore emulator and run with `FIRESTORE_EMULATOR_HOST=127.0.0.1:8090` — no GCP credentials needed. Emulator: `npx firebase-tools@13 emulators:start --only firestore --project pnote` (Java required; a minimal `firebase.json` with `emulators.firestore.port` is enough, e.g. in a scratch dir). |
...
- Production Firebase/GCP project is **`pnote`**. Live app: https://pnote.web.app. Production Firestore must exist in `pnote` (Native mode). The deploy workflow best-effort-creates it; if the DB is missing or the Cloud Run SA lacks `roles/datastore.user`, `/api/db-status` returns 503 and the app runs offline (localStorage only). Hosting rewrites `/api/**` to Cloud Run `p-note-api` (same project).
